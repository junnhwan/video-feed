...
import type { Account, MessageResponse, ProfileResponse, TokenResponse } from './types'

export function register(username: string, password: string) {
  return postJson<TokenResponse>('/account/register', { username, password })
}
...
export function getProfile(accountId: number) {
  return postJson<ProfileResponse>('/account/getProfile', { account_id: accountId })
}
